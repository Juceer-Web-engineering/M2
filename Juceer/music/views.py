...
class music_pop_year_list(APIView):
    renderer_classes = [JSONRenderer, TemplateHTMLRenderer]
    template_name = 'music/list_artist.html'
    ...
    def get(self, request, number = None, song_year= None, format = None):
        if (int(number) == 0):
            raise Http404
        if (number == None and song_year == None):
            music = MusicPost.objects.order_by('artist_hotttnesss').reverse().values('artist_id', 'artist_name', 'artist_info').distinct()
            title = "List of artists by popularity"
            serializer = MusicSerializer(music, many=True)
            ...
            music = MusicPost.objects.filter(song_year = song_year).order_by('artist_hotttnesss').reverse().values('artist_id', 'artist_name', 'artist_info').distinct()
            title = "List of artists by popularity"
            serializer = MusicSerializer(music, many=True)
        return Response({'object_list': serializer.data,  'title': title})
...
class music_detail(APIView):
    ...
    def delete (self, request, artist_id, format = None):
        music = MusicPost.objects.filter(artist_id = artist_id).all()
        music.delete()
...
class songs_list(APIView):
    ...
    def get(self, request, artist_id = None, song_year = None, format = None):
        ...
        if (artist_id == None and song_year == None):
            music = MusicPost.objects.all()
            title = "List of songs"
        elif(artist_id != None and song_year == None):
            music = MusicPost.objects.filter(artist_id = artist_id).order_by('song_year')
            if(len(music)==0):
                raise Http404
            title = "List of songs of "+ music[0].artist_name
        elif(artist_id != None and song_year != None):
...
class stat_mean(APIView):
    ...
    def get(self, request, artist_id = None, format = None):
        ...
        "year_by_year_mean": 'http://127.0.0.1:8000/juceer/0.1/statistics/mean/artists/'+ str(artist_id) + '/years/',
...
class stat_median(APIView):
    ...
    def get(self, request, artist_id = None, format = None):
        ...
        "std_deviation": 'http://127.0.0.1:8000/juceer/0.1/statistics/std_dev/artists/'+ str(artist_id) + '/',
        "year_by_year_mean": 'http://127.0.0.1:8000/juceer/0.1/statistics/median/artists/'+ str(artist_id) + '/years/',
        "songs_of_the_same_artist": 'http://127.0.0.1:8000/juceer/0.1/artists/' + str(artist_id) + '/songs/',
        }
...
class stat_std_dev(APIView):
    ...
    def get(self, request, artist_id = None, format = None):
        ...
        "year_by_year_mean": 'http://127.0.0.1:8000/juceer/0.1/statistics/std_dev/artists/'+ str(artist_id) + '/years/',
        "songs_of_the_same_artist": 'http://127.0.0.1:8000/juceer/0.1/artists/' + str(artist_id) + '/songs/',
        }
